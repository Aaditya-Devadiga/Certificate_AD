...
function setupGeneratorGate() {
  const modal = document.getElementById("generatorGateModal");
  const continueLink = document.getElementById("generatorGateContinue");
  if (!modal || !continueLink) return;

  const open = (destination) => {
    if (destination) continueLink.setAttribute("href", destination);
  ...
  document.querySelectorAll('a[data-generator-gate="true"]').forEach((link) => {
  ...
  modal.querySelectorAll("[data-close-generator-gate]").forEach((node) => {
